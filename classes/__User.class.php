	...
	public function login($array){
        if(isset($array) && is_array($array) && count($array) > 0){
            if(!isset($array['userId']) || trim($array['userId']) == ''){
                ...
                if($result = $this->query("SELECT `id`, `userId`, `isActive` FROM `tblUser` WHERE `userId` = '" . trim($array['userId']) . "' AND `password` = '" . trim($array['password']) . "' AND `isDelete` = 'N'")){
    ...
    public function adminLogin($array){
        if(isset($array) && is_array($array) && count($array) > 0){
            if(!isset($array['userId']) || trim($array['userId']) == ''){
                ...
                // Take action to check login from Database
                if($result = $this->query("SELECT `id`, `userId`, `groupId`, `isActive` FROM `tblUser` WHERE `userId` = '" . trim($array['userId']) . "' AND `password` = '" . trim($array['password']) . "' AND `isDelete` = 'N'")){
                    if(is_array($result) && count($result) > 0){
