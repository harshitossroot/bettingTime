...
if(isset($_POST) && is_array($_POST) && count($_POST) > 0 && isset($_POST['_COMMENT_CAT']) && $_POST['_COMMENT_CAT'] == 'SADARI_COMMENT' ){
	if(!$User->checkLoginStatus()){
		...
		if($Card->addSadariSportsComments($_POST, $reqID['1'])){
    		//C::redirect(C::link('sadariDetails.php', false, true));
				...
				<div class="details-page">
					...
					<table class="ask-table">
						...
						<tr>
							...
							<th class="text-yellow font18 text-center"><?php echo $_SESSION['value']['0']['maximumBetting']; ?></th>
					...
					<div class="col-lg-9 col-md-9" id="show-pop-up">
						...
						<div class="ask-page-content ask-land-page-content">
							<div class="ask-page-content-header">
								...
								<p class="custom-text custom-p">Have troubles with Sports? <a href="submitComplaint.php">Submit a complaint</a> or <a href="">Learn more</a> .  </p>
							</div>
							<div class="ask-page-content-body">
							<?php
								...
								<div class="row content">
									<table class="ask-table text-bold custom-table-padding">
										<tr>
											...
											<td><a href=""><?php echo $_SESSION['value'][0]['sportsName']; ?></a></td>
										</tr>
										<tr>
											<td> Join Code :</td>
											<td><a href=""><?php echo $_SESSION['value'][0]['joinCode']; ?></a></td>
										</tr>
										<tr>
											<td> Official Website :</td>
											<td><a href=""><?php echo $_SESSION['value'][0]['link']; ?></a></td>
										</tr>
										<tr>
											<td>Wager :</td>
											<td><a href=""><?php echo $_SESSION['value'][0]['wager']; ?></a></td>
										</tr>
										<tr>
										...
										<tr>
											<td>Sadari Odd :</td>
											<td><a href=""><?php echo $_SESSION['value'][0]['sadariOdd']; ?></a></td>
										</tr>
										<tr>
											<td>Closing Time :</td>
											<td><a href=""><?php echo $_SESSION['value'][0]['closingTime']; ?> sec</a></td>
										</tr>
										<?php
											if(isset($value[0]['sportsOtherDetails'])){
						...
						<div class="ask-page-content">
							...
							<div class="ask-page-content-body-details">
								...
								<?php
								$result = $User->query("SELECT `TSC`.`gdComments`, `TSC`.`badComments`, `TSC`.`rating`, `TSC`.`updatedOn`, `TU`.`userId` FROM `tblSadariSportsComment` as `TSC`, `tblUser` as `TU`  WHERE `TSC`.`isRecommanded` = 'Y' AND `TSC`.`userId` = `TU`.`id` AND `TSC`.`sportsId` = '" . $reqID[1] . "'");
								if(is_array($result) && count($result) > 0){
									$index = 0;
								?>
