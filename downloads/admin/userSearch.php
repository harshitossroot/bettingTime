				...
				<thead>
					...
					<tbody>
						...
						if(strlen($query) >= $min_length){
							...
							$result = $Base->query("SELECT `id`, `email`, `nickName`, `userId`, `password`, `updatedOn`, `groupId` FROM `tblUser` WHERE (`userId` LIKE '%" . $query . "%') OR (`nickName` LIKE '%" . $query . "%') OR (`email` LIKE '%" . $query . "%')");
