...
$userDetails = (isset($_SESSION['userDetails']) ? $_SESSION['userDetails'] : array());
...
function getReceiverChats($userId, $receiverId, $limit = 50, $offset = 0){
	...
	if(is_array($receiverMessages) && count($receiverMessages) > 0){
		foreach($receiverMessages as $index => $messageFile){
			...
			if(is_array($content) && count($content) > 0){
				...
				foreach($content as $messageIndex => $messageDetails){
					...
					if(!isset($userDetails[(int)$messageDetails['_sender']]))
						$userDetails[(int)$messageDetails['_sender']] = User::name((int)$messageDetails['_sender']);
					...
					if($messageDetails['_sender'] == User::loggedInUserId()){
						...
						$userName = $userDetails[(int)$messageDetails['_sender']];
						$class = 'right';
						$class1 = 'friend-with-a-SVAGina';
						$class2 = 'left';
						$read = 'read';
						if($messageDetails['_read'] == 'N')
							$read = 'unread';
     ...
     //                    </div>';
					$array['chats'][] = '<li class="chat-msg-' . $read . ' ' . $class1 . ' ' . $class . ' __message_' . $userId . '_' . $receiverId . '_' . $messageIndex . '" data-limit="' . $limit . '" data-index="' . $messageIndex . '" data-file="' . $messageFile . '">
											<div class="head">
												<span class="time">' . $messageTime . '</span>
												<span class="name">' . $userName . '</span>
											</div>
											<div class="message">' . $messageDetails['_message'] . '</div>
										</li>';
...
if(isset($_GET['a']) && $_GET['a'] == 'READ' && isset($_GET['u']) && (int)$_GET['u'] > 0){
	...
	if(is_array($receiverDirs) && count($receiverDirs) > 0){
		foreach($receiverDirs as $index => $receiverId){
			...
			$chatMessages['chats']['___chatBoxBody_' . $chatUser . '_' . $receiverId]['chats'] = implode(chr(13), $messages['chats']);
			$chatMessages['chats']['___chatBoxBody_' . $chatUser . '_' . $receiverId]['chatid'] = '.' . implode(',.', $messages['chatsId']);
		}
	}
